...
function KeywordFilter({ metadata, filter, setFilterValue }) {
  ...
  const filteredKeywords = metadata.keywords.filter(
    (keyword) => descriptionTokens.keyword[keyword.slug]
  )
...
export default connect(mapStateToProps, { setFilterValue })(KeywordFilter)
